...
Matl_MagnMech_Gent::Matl_MagnMech_Gent()
{
...
int Matl_MagnMech_Gent::computeStressAndTangent(bool tangFlag, int sss,  MatrixXd&  Fn, MatrixXd&  F, double& pres, VectorXd&  stre, MatrixXd&  Cmat, InternalVariables& ivar, int gp, double dt)
{
...
/*
    for(i1=0; i1<9; i1++)
    {
        ...
        for(j1=0; j1<9; j1++)
        {
            ...
            //Cmat(i1,j1) -= (streMagn(j,l)*KronDelta(i,k));
...
int Matl_MagnMech_Gent::computeStressAndTangent(bool tangFlag, int sss,  MatrixXd&  Fn, MatrixXd&  F, VectorXd&  magnField, double& pres, VectorXd&  stre, VectorXd&  magnDisp, MatrixXd&  Cmat, MatrixXd&  Bmat, MatrixXd&  Amat, InternalVariables& ivar, int gp, double dt)
{
...
int Matl_MagnMech_Gent::computeMagneticComponents(VectorXd&  magnField, VectorXd&  magnDisp, MatrixXd&  Amat)
{
    double  mu0   = data_magnfield[0];      // permeability
...
int Matl_MagnMech_Gent::computeMagneticDisplacement(VectorXd&  magnField, VectorXd&  magnDisp)
{
    double  mu0   = data_magnfield[0];      // permeability
...
int Matl_MagnMech_Gent::computeMechanicalStress(MatrixXd&  F, double&  pres, VectorXd&  stre)
{
    double  r1d3 = 1.0/3.0;
    double  r2d3 = 2.0/3.0;
    double  r5d3 = 5.0/3.0;
...
int Matl_MagnMech_Gent::computeMaxwellStress(VectorXd&  magnField, VectorXd&  stre)
{
...
int Matl_MagnMech_Gent::computeTotalStress(MatrixXd&  F, VectorXd&  magnField, double& pres, VectorXd&  stre)
{
    double  mu0   = matData[3]; // permittivity
