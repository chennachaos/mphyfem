...
#include "MagnetoMech3DWedge221.h"
#include "MyTime.h"
...
MagnetoMech3DWedge221::MagnetoMech3DWedge221()
{
  ndim   = 3;
  degree = 2;
  npElem = 10;
...
void MagnetoMech3DWedge221::prepareElemData()
{
...
double MagnetoMech3DWedge221::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int MagnetoMech3DWedge221::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int MagnetoMech3DWedge221::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
double  MagnetoMech3DWedge221::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int MagnetoMech3DWedge221::calcMassMatrixPressure(MatrixXd& Mlocal, bool MassLumping)
{
...
int MagnetoMech3DWedge221::calcStiffnessAndResidualMixed2(MatrixXd& Kup, MatrixXd& Kpu, MatrixXd& Kpp, VectorXd& FlocalU,  VectorXd& FlocalP, bool firstIter)
{
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        F(0) = computeValuePrev(0, dNu_dx) + 1.0;            // eps_xx
        F(1) = computeValuePrev(1, dNu_dx);                  // eps_yx
        F(2) = computeValuePrev(2, dNu_dx);                  // eps_zx
        F(3) = computeValuePrev(0, dNu_dy);                  // eps_xy
        F(4) = computeValuePrev(1, dNu_dy) + 1.0;            // eps_yy
        F(5) = computeValuePrev(2, dNu_dy);                  // eps_zy
        F(6) = computeValuePrev(0, dNu_dz);                  // eps_xz
        F(7) = computeValuePrev(1, dNu_dz);                  // eps_yz
        F(8) = computeValuePrev(2, dNu_dz) + 1.0;            // eps_zz
...
int  MagnetoMech3DWedge221::calcStiffnessAndResidualElecField(MatrixXd& Kff, VectorXd& FlocalF, bool firstIter)
{
...
int MagnetoMech3DWedge221::calcResidual(VectorXd& Flocal)
{
...
int MagnetoMech3DWedge221::calcInternalForces()
{
  return 0;
}
