...
GrowthElem3DWedge21::GrowthElem3DWedge21()
{
  ELEM_SHAPE = ELEM_SHAPE_WEDGE_BERNSTEIN;
...
void GrowthElem3DWedge21::prepareElemData()
{
...
double GrowthElem3DWedge21::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int GrowthElem3DWedge21::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
    ...
    if(MassLumping)
    {
      double fact = elmDat[5]*computeVolume(true)/18.0;
...
int GrowthElem3DWedge21::calcMassMatrixPressure(MatrixXd& Mlocal, bool MassLumping)
{
    if( (Mlocal.rows() != 6) || (Mlocal.cols() != 6) )
      Mlocal.resize(6, 6);
    Mlocal.setZero();
    ...
    if(MassLumping)
    {
      double fact = computeVolume(true)/6.0;
...
double  GrowthElem3DWedge21::calcCriticalTimeStep(bool flag)
{
    int  ii, jj, gp;
/*
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        F[6] = computeValueCur(2, dN_dz) + 1.0;

        detF = F[0]*(F[4]*F[6]-F[5]*F[7]) - F[3]*(F[1]*F[6]-F[2]*F[7]) + F[6]*(F[1]*F[5]-F[2]*F[4]);

        Ib = (F[0]*F[0]+F[3]*F[3]+F[6]*F[6]) + (F[1]*F[1]+F[4]*F[4]+F[7]*F[7]) + (F[2]*F[2]+F[5]*F[5]+F[6]*F[6]);
...
*/
    ...
    double xNode[18], yNode[18], zNode[18];
...
int GrowthElem3DWedge21::calcStiffnessAndResidualMixed2(MatrixXd& Kup, MatrixXd& Kpu, MatrixXd& Kpp, VectorXd& Flocal1, VectorXd& Flocal2, bool firstIter)
{
    ...
    double xNode[18], yNode[18], zNode[18], xx, yy, zz;
    for(ii=0;ii<npElem;ii++)
    {
...
int GrowthElem3DWedge21::calcResidual(VectorXd& Flocal)
{
...
int GrowthElem3DWedge21::calcResidualPressure(VectorXd& Flocal)
{
    int gp, ii;
    VectorXd  N(nlbfU), dN_dx(nlbfU), dN_dy(nlbfU), dN_dz(nlbfU);
...
int GrowthElem3DWedge21::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
int GrowthElem3DWedge21::calcInternalForces()
{
  return 0;
}
...
void GrowthElem3DWedge21::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void GrowthElem3DWedge21::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void GrowthElem3DWedge21::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
    double  detF, fact, dvol, dvol0, Jac, param[3], Np[6], r1d3 = 1.0/3.0;
 
    MatrixXd  Fg(3,3), FgInv(3,3), F(3,3), Fe(3,3);
    double  grp1 = 1.0 + elmDat[10]*timeFunction[0].prop ;
    growthfunction_isotropic(1, ndim, grp1, Fg);
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        F(2,2) = computeValue(2, dN_dz) + 1.0;
...
void GrowthElem3DWedge21::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void GrowthElem3DWedge21::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
