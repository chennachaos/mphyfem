...
#include "BernsteinElem3DElecMechHex110.h"
#include "MyTime.h"
...
BernsteinElem3DElecMechHex110::BernsteinElem3DElecMechHex110()
{
  ndim   = 3;
  degree = 1;
  npElem = 8;
...
void BernsteinElem3DElecMechHex110::prepareElemData()
{
...
double BernsteinElem3DElecMechHex110::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int BernsteinElem3DElecMechHex110::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int BernsteinElem3DElecMechHex110::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
double  BernsteinElem3DElecMechHex110::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int BernsteinElem3DElecMechHex110::calcMassMatrixPressure(MatrixXd& Mlocal, bool MassLumping)
{
...
int  BernsteinElem3DElecMechHex110::calcStiffnessAndResidual(MatrixXd& Kuu, MatrixXd& Kuf, MatrixXd& Kfu, MatrixXd& Kff, VectorXd& FlocalU, VectorXd& FlocalF, bool firstIter)
{
    ...
    double  detF, detFn, Jhat, trF, fact, fact1, dvol, dvol0, Jac, volstrain, phi, pbar, r2d3=2.0/3.0;
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        //dUdJ   = volumetricfunctions_getFirstDerivative(1, BULK, 0.0, detF);
        //d2UdJ2 = volumetricfunctions_getSecondDerivative(1, BULK, 0.0, detF);
        //d2UdJ2 *= eps;

        cout << "This needs to be updated" << endl;
        //dUdJ   = volumetricfunctions_getFirstDerivative(8, BULK, detFn);
        //d2UdJ2 = volumetricfunctions_getSecondDerivative(8, BULK, detFn);
        ...
        for(ii=0; ii<nlbfU; ii++)
        {
          ...
          //Kpu(0, TI  )  += (af*d2UdJ2*bb1);
          //Kpu(0, TIp1)  += (af*d2UdJ2*bb2);
          //Kpu(0, TIp2)  += (af*d2UdJ2*bb3);
        ...
        Kpp(0,0) -= (af*dvol0/d2UdJ2);

        if(finite)
        {
          //Rp  -= (detF - 1.0 - pres*eps)*dvol0;

          //Rp  -= (dUdJ - pres)*dvol0;

          //Rp  -= (dUdJ - pres)*dvol0*eps;

          //Rp  -= (dUdJ - pres)*dvol0/d2UdJ2;

          Rp(0)  -= ( detF - Jhat - pres/d2UdJ2 )*dvol0;
        }
        else
        {
...
int BernsteinElem3DElecMechHex110::solveForPressure()
{
...
int BernsteinElem3DElecMechHex110::calcStiffnessAndResidualMixed2(MatrixXd& Kup, MatrixXd& Kpu, MatrixXd& Kpp, VectorXd& Flocal1, VectorXd& Flocal2, bool firstIter)
{
...
int BernsteinElem3DElecMechHex110::calcResidual(VectorXd& Flocal)
{
...
int BernsteinElem3DElecMechHex110::calcInternalForces()
{
  return 0;
}
...
void BernsteinElem3DElecMechHex110::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex110::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex110::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
...
void BernsteinElem3DElecMechHex110::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void BernsteinElem3DElecMechHex110::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
