...
#include "BernsteinElem3DElecMechTet22F.h"
#include "MyTime.h"
...
BernsteinElem3DElecMechTet22F::BernsteinElem3DElecMechTet22F()
{
  ndim   = 3;
  degree = 2;
  npElem = 10;
...
void BernsteinElem3DElecMechTet22F::prepareElemData()
{
...
double BernsteinElem3DElecMechTet22F::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int BernsteinElem3DElecMechTet22F::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int BernsteinElem3DElecMechTet22F::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
double  BernsteinElem3DElecMechTet22F::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int BernsteinElem3DElecMechTet22F::calcStiffnessAndResidual(MatrixXd& Kuu, MatrixXd& Kuf, MatrixXd& Kfu, MatrixXd& Kff, VectorXd& FlocalU, VectorXd& FlocalF, bool firstIter)
{
    ...
    // values at the element centroid
    ...
    GeomData->computeBasisFunctions3D(CONFIG_ORIGINAL, ELEM_SHAPE_TETRA_BERNSTEIN, degree, param, nodeNums, &Nuc(0), &dNuc_dx(0), &dNuc_dy(0), &dNuc_dz(0), Jac);

    Fc(0,0) = computeValueCur(0, dNuc_dx) + 1.0;            // eps_xx
    Fc(0,1) = computeValueCur(0, dNuc_dy);                  // eps_xy
    Fc(0,2) = computeValueCur(0, dNuc_dz);                  // eps_xz
    Fc(1,0) = computeValueCur(1, dNuc_dx);                  // eps_yx
    Fc(1,1) = computeValueCur(1, dNuc_dy) + 1.0;            // eps_yy
    Fc(1,2) = computeValueCur(1, dNuc_dz);                  // eps_yz
    Fc(2,0) = computeValueCur(2, dNuc_dx);                  // eps_zx
    Fc(2,1) = computeValueCur(2, dNuc_dy);                  // eps_zy
    Fc(2,2) = computeValueCur(2, dNuc_dz) + 1.0;            // eps_zz

    trFc  = Fc.trace();
    detFc = F.determinant();

    // basis functions wrt to the current configuration
    GeomData->computeBasisFunctions3D(CONFIG_DEFORMED, ELEM_SHAPE_TETRA_BERNSTEIN, degree, param, nodeNums, &Nuc(0), &dNuc_dx(0), &dNuc_dy(0), &dNuc_dz(0), Jac);
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        for(ii=0; ii<nlbfU; ii++)
        {
          ...
          for(jj=0; jj<nlbfU; jj++)
          {
            ...
            cc3 = af*(dNuc_dz(jj) - cc3);

            Kuu(TI,   TJ)    +=  tarr[0]*cc1;
            Kuu(TI,   TJp1)  +=  tarr[0]*cc2;
            Kuu(TI,   TJp2)  +=  tarr[0]*cc3;

            Kuu(TIp1, TJ)    +=  tarr[1]*cc1;
            Kuu(TIp1, TJp1)  +=  tarr[1]*cc2;
            Kuu(TIp1, TJp2)  +=  tarr[1]*cc3;

            Kuu(TIp2, TJ)    +=  tarr[2]*cc1;
            Kuu(TIp2, TJp1)  +=  tarr[2]*cc2;
            Kuu(TIp2, TJp2)  +=  tarr[2]*cc3;
          }
          //printMatrix(Kuu);
...
int BernsteinElem3DElecMechTet22F::calcResidual(VectorXd& Flocal)
{
...
int BernsteinElem3DElecMechTet22F::calcInternalForces()
{
  return 0;
}
...
void BernsteinElem3DElecMechTet22F::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechTet22F::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechTet22F::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
    double  detF, detFc, trFc, dvol, Jac, fact, dvol0, param[3];
    MatrixXd  F(3,3), Fn(3,3), Fc(3,3), Cmat(9,9), Bmat(3,9), Amat(3,3);
    ...
    Fc(0,0) = computeValue(0, dNuc_dx) + 1.0;            // eps_xx
    Fc(0,1) = computeValue(0, dNuc_dy);                  // eps_xy
    Fc(0,2) = computeValue(0, dNuc_dz);                  // eps_xz
    Fc(1,0) = computeValue(1, dNuc_dx);                  // eps_yx
    Fc(1,1) = computeValue(1, dNuc_dy) + 1.0;            // eps_yy
    Fc(1,2) = computeValue(1, dNuc_dz);                  // eps_yz
    Fc(2,0) = computeValue(2, dNuc_dx);                  // eps_zx
    Fc(2,1) = computeValue(2, dNuc_dy);                  // eps_zy
    Fc(2,2) = computeValue(2, dNuc_dz) + 1.0;            // eps_zz
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        F(0,1) = computeValue(0, dNu_dy);                  // eps_xy
        F(0,2) = computeValue(0, dNu_dz);                  // eps_xz
        F(1,0) = computeValue(1, dNu_dx);                  // eps_yx
        F(1,1) = computeValue(1, dNu_dy) + 1.0;            // eps_yy
        F(1,2) = computeValue(1, dNu_dz);                  // eps_yz
        F(2,0) = computeValue(2, dNu_dx);                  // eps_zx
        F(2,1) = computeValue(2, dNu_dy);                  // eps_zy
        F(2,2) = computeValue(2, dNu_dz) + 1.0;            // eps_zz
        ...
        //{
          ...
          //elecField[2] -= (dNu_dz[ii]*SolnData->var3[nodeNums[ii]]);
        //}
...
void BernsteinElem3DElecMechTet22F::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void BernsteinElem3DElecMechTet22F::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
