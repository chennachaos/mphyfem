...
#include "BernsteinElem2DElecMechTri220.h"
#include "MyTime.h"
...
BernsteinElem2DElecMechTri220::BernsteinElem2DElecMechTri220()
{
  ndim   = 2;
  degree = 2;
  npElem = 6;
...
void BernsteinElem2DElecMechTri220::prepareElemData()
{
...
double BernsteinElem2DElecMechTri220::computeVolume(bool configflag)
{
...
int BernsteinElem2DElecMechTri220::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int BernsteinElem2DElecMechTri220::calcMassMatrixPressure(MatrixXd& Mlocal, bool MassLumping)
{
...
double  BernsteinElem2DElecMechTri220::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int BernsteinElem2DElecMechTri220::calcStiffnessAndResidual(MatrixXd& Kuu, MatrixXd& Kuf, MatrixXd& Kfu, MatrixXd& Kff, VectorXd& FlocalU, VectorXd& FlocalF, bool firstIter)
{
...
int BernsteinElem2DElecMechTri220::solveForPressure()
{
...
int BernsteinElem2DElecMechTri220::calcStiffnessAndResidualMixed2(MatrixXd& Kup, MatrixXd& Kpu, MatrixXd& Kpp, VectorXd& Flocal1, VectorXd& Flocal2, bool firstIter)
{
    int   err,  isw,  count,  count1, index, ii, jj, kk, ll, mm, gp, TI, TIp1, TJ, TJp1;
...
int BernsteinElem2DElecMechTri220::calcResidual(VectorXd& Flocal)
{
    int   err,  isw,  count,  count1, index, ii, jj, kk, ll, mm, gp, TI, TIp1, TJ, TJp1;
...
int BernsteinElem2DElecMechTri220::calcResidualPressure(VectorXd& Flocal)
{
    int gp, ii;
    VectorXd  N(nlbfU), dN_dx(nlbfU), dN_dy(nlbfU);
...
/*
int BernsteinElem2DElecMechTri220::calcResidualTIC(VectorXd& Flocal)
{
    int   err,  isw,  count,  count1, index, ii, jj, kk, ll, mm, gp, TI, TIp1, TJ, TJp1;
...
int  BernsteinElem2DElecMechTri220::solveForPressureTIC(VectorXd& matK1, double beta, double dt, VectorXd& Flocal)
{
    double  volstrain = 0.0, volstrainPrev=0.0;
...
int BernsteinElem2DElecMechTri220::toComputeInfSupCondition(MatrixXd& Kuu, MatrixXd& Kup, MatrixXd& Kpp)
{
    // to compute the inf-sup constant

    int  ii, jj, kk, gp, TI, TIp1, TJ, TJp1, sizep=1;
    VectorXd  N(nlbfU), dN_dx(nlbfU), dN_dy(nlbfU);

    double  fact, dvol0, Jac, bb1, bb2, bb3, bb4, bb5, param[2];
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        for(ii=0;ii<nlbfU;ii++)
        {
          ...
          for(jj=0; jj<nlbfU; jj++)
          {
            ...
            fact = bb1*dN_dx[jj] + bb2*dN_dy[jj] ;
...
int BernsteinElem2DElecMechTri220::calcLoadVector(VectorXd& Flocal)
{
...
int BernsteinElem2DElecMechTri220::calcInternalForces()
{
...
void BernsteinElem2DElecMechTri220::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem2DElecMechTri220::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem2DElecMechTri220::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
...
void BernsteinElem2DElecMechTri220::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void BernsteinElem2DElecMechTri220::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
