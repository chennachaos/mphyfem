...
#include "BernsteinElem3DElecMechHex221P.h"
#include "MyTime.h"
...
BernsteinElem3DElecMechHex221P::BernsteinElem3DElecMechHex221P()
{
  ndim   = 3;
  degree = 2;
  npElem = 27;
  ...
  Kpu.resize(nlbfP, nsize);

  Kpp.resize(nlbfP, nlbfP);
  Rp.resize(nlbfP);
  KppInv.resize(nlbfP, nlbfP);

  presDOF.resize(nlbfP);
  presDOF.setZero();
}
...
void BernsteinElem3DElecMechHex221P::prepareElemData()
{
...
double BernsteinElem3DElecMechHex221P::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int BernsteinElem3DElecMechHex221P::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int BernsteinElem3DElecMechHex221P::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
double  BernsteinElem3DElecMechHex221P::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int BernsteinElem3DElecMechHex221P::calcMassMatrixPressure(MatrixXd& Mlocal, bool MassLumping)
{
...
int  BernsteinElem3DElecMechHex221P::calcStiffnessAndResidual(MatrixXd& Kuu, MatrixXd& Kuf, MatrixXd& Kfu, MatrixXd& Kff, VectorXd& FlocalU, VectorXd& FlocalF, bool firstIter)
{
    ...
    double  detF, detFn, trF, fact, fact1, dvol, dvol0, Jac, volstrain, phi, pbar, r2d3=2.0/3.0;
    double  bb1, bb2, bb3, bb4, bb5, cc1, cc2, cc3, cc4, cc5, dUdJ, d2UdJ2, Jhat;
    ...
    for(gp=0; gp<nGP; gp++)
    {
        ...
        BernsteinBasisFunsTetra(1, 0.5*(1.0+param[0]), 0.5*(1.0+param[1]), 0.5*(1.0+param[2]), Np);
        pres = 0.0;
        for(ii=0; ii<nlbfP; ii++)
        {
          pres += (Np[ii]*presDOF[ii]);
        }
        ...
        //dUdJ   = volumetricfunctions_getFirstDerivative(1, BULK, detF);
        //d2UdJ2 = volumetricfunctions_getSecondDerivative(1, BULK, detF);
        //d2UdJ2 *= eps;

        cout << "This needs to be updated" << endl;
        //dUdJ   = volumetricfunctions_getFirstDerivative(1, BULK, detFn);
        //d2UdJ2 = volumetricfunctions_getSecondDerivative(1, BULK, detFn);

        Jhat = detFn - dUdJ/d2UdJ2;
        ...
        if(finite)
        {
          ...
          //fact  = (dUdJ - pres)/d2UdJ2;

          //fact  =  detF - Jhat - pres/d2UdJ2;
        }
        else
        {
...
int BernsteinElem3DElecMechHex221P::solveForPressure()
{
    ...
    else // implicit scheme
    {
      ...
      for(ii=0; ii<npElem; ii++)
      {
        ...
        dispIncr(ind1+0)  =  SolnData->var1Incr[ind2+0];
        dispIncr(ind1+1)  =  SolnData->var1Incr[ind2+1];
        dispIncr(ind1+2)  =  SolnData->var1Incr[ind2+2];
      }

      vtemp = Kpu*dispIncr;

      presDOF += KppInv*(-Rp - vtemp);
    }
...
int BernsteinElem3DElecMechHex221P::calcStiffnessAndResidualMixed2(MatrixXd& Kup, MatrixXd& Kpu, MatrixXd& Kpp, VectorXd& FlocalU,  VectorXd& FlocalP, bool firstIter)
{
    int   err,  isw,  count,  count1, index, ii, jj, kk, ll, mm, gp, TI, TIp1, TIp2, TJ, TJp1, TJp2;
...
int  BernsteinElem3DElecMechHex221P::calcStiffnessAndResidualElecField(MatrixXd& Kff, VectorXd& FlocalF, bool firstIter)
{
    ...
    MatrixXd  Amat(3,3);
    Amat.setZero();
    VectorXd  elecDisp(3),  elecField(3);
...
int BernsteinElem3DElecMechHex221P::calcResidual(VectorXd& Flocal)
{
...
int BernsteinElem3DElecMechHex221P::calcInternalForces()
{
  return 0;
}
...
void BernsteinElem3DElecMechHex221P::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex221P::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex221P::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
...
void BernsteinElem3DElecMechHex221P::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void BernsteinElem3DElecMechHex221P::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
