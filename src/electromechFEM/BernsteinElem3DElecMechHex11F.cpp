...
#include "BernsteinElem3DElecMechHex11F.h"
#include "MyTime.h"
...
BernsteinElem3DElecMechHex11F::BernsteinElem3DElecMechHex11F()
{
  ndim   = 3;
  degree = 1;
  npElem = 8;
...
void BernsteinElem3DElecMechHex11F::prepareElemData()
{
...
double BernsteinElem3DElecMechHex11F::computeVolume(bool configflag)
{
    double  Jac, param[3];
...
int BernsteinElem3DElecMechHex11F::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
...
int BernsteinElem3DElecMechHex11F::calcLoadVector(VectorXd& Flocal)
{
  return 0;
}
...
double  BernsteinElem3DElecMechHex11F::calcCriticalTimeStep(bool flag)
{
    int  ii;
...
int BernsteinElem3DElecMechHex11F::calcStiffnessAndResidual(MatrixXd& Kuu, MatrixXd& Kuf, MatrixXd& Kfu, MatrixXd& Kff, VectorXd& FlocalU, VectorXd& FlocalF, bool firstIter)
{
    ...
    VectorXd  stre(9),  elecDisp(3),  elecField(3),  Fc(9), qVec(9);
    stre.setZero();
    ...
    GeomData->computeBasisFunctions3D(CONFIG_ORIGINAL, ELEM_SHAPE_HEXA, degree, param, nodeNums, &Nuc(0), &dNuc_dx(0), &dNuc_dy(0), &dNuc_dz(0), Jac);
    ...
    trFc  = F(0,0)+F(1,1)+F(2,2);
    detFc = F.determinant();

    // basis functions wrt to the current configuration
    GeomData->computeBasisFunctions3D(CONFIG_DEFORMED, ELEM_SHAPE_HEXA, degree, param, nodeNums, &Nuc(0), &dNuc_dx(0), &dNuc_dy(0), &dNuc_dz(0), Jac);
...
int BernsteinElem3DElecMechHex11F::calcResidual(VectorXd& Flocal)
{
...
int BernsteinElem3DElecMechHex11F::calcInternalForces()
{
  return 0;
}
...
void BernsteinElem3DElecMechHex11F::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex11F::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void BernsteinElem3DElecMechHex11F::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
...
void BernsteinElem3DElecMechHex11F::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void BernsteinElem3DElecMechHex11F::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
