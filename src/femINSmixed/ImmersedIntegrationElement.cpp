...
void ImmersedIntegrationElement::initialiseDOFvalues()
{
  ...
  elemNums.resize(nnode);
  forAssyVec.resize(nnode*ndim);
  for(int ii=0; ii<nnode*ndim; ii++)
    forAssyVec[ii] = -1;
...
/*
  ...
  {
    for(jj=0;jj<ndim;jj++)
      pt[jj] += (GeomDataLag->NodePosCur[nodeNums[ii]][jj] * N[ii] );
  }
*/
  return;
}
...
/*
  ...
  {
    for(jj=0; jj<ndim; jj++)
      velSpec[jj] += (GeomDataLag->specValNew[nodeNums[ii]][jj] * NN[ii] );
  }
*/
  return;
}
...
/*
  ...
  {
    for(jj=0; jj<ndim; jj++)
      velSpec[jj] += (GeomDataLag->specValCur[nodeNums[ii]][jj] * NN[ii] );
  }
*/
  return;
}
...
/*
  ...
  {
    for(jj=0; jj<ndim; jj++)
      velSpec[jj] += (GeomDataLag->acceNew[nodeNums[ii]][jj] * NN[ii] );
  }
*/
  return;
}
...
/*
  ...
  {
    for(jj=0; jj<ndim; jj++)
      velSpec[jj] += (GeomDataLag->acceCur[nodeNums[ii]][jj] * NN[ii] );
  }
*/
  return;
}
...
/*
  ...
  int  gp, ii, jj, kk, nlb = nodeNums.size();
  double  dvol, detJ;
  ...
  for(ii=0;ii<nlb;ii++)
  {
    xx[ii] = GeomDataLag->NodePosCur[nodeNums[ii]][0];
    yy[ii] = GeomDataLag->NodePosCur[nodeNums[ii]][1];
    zz[ii] = 0.0;
  }
  ...
  {
    for(ii=0;ii<nlb;ii++)
      zz[ii] = GeomDataLag->NodePosCur[nodeNums[ii]][2];
...
/*
  ...
  Kh.resize(size, nlb*ndim);
  Kh.setZero();

  Kv.resize(nlb*ndim, size);
  Kv.setZero();
...
/*
  ...
  for(ii=0;ii<nlbL;ii++)
  {
    xNode[ii] = GeomDataLag->NodePosCur[nodeNums[ii]][0];
    yNode[ii] = GeomDataLag->NodePosCur[nodeNums[ii]][1];
  }
...
/*
  ...
  Kh.resize(3*nlbS, ndim*nlbL);
  Kh.setZero();

  Kv.resize(ndim*nlbS, 3*nlbS);
  Kv.setZero();
