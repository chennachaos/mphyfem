...
void BernsteinElem3DINSTetra10Node::prepareElemData(vector<myPoint>& node_coords)
{
    ...
    for(ii=0; ii<nGP; ii++)
    {
      ...
      dNvdz[ii].resize(nlbfU);

      Np[ii].resize(nlbfU);
      dNpdx[ii].resize(nlbfU);
      dNpdy[ii].resize(nlbfU);
      dNpdz[ii].resize(nlbfU);
...
double  BernsteinElem3DINSTetra10Node::ResidualIncNavStokesAlgo1(vector<myPoint>& node_coods, double* elemData, double* timeData, VectorXd& veloVec, VectorXd& veloVecPrev, VectorXd& acceVec, VectorXd& acceVecPrev, VectorXd& presVec, VectorXd& presVecPrev, VectorXd&  Flocal1, VectorXd&  Flocal2, double timeCur)
{
...
int  BernsteinElem3DINSTetra10Node::ResidualIncNavStokesAlgo2(vector<myPoint>& node_coords, double* elemData, double* timeData, VectorXd& veloPrev, VectorXd& accePrev, VectorXd& presPrev, VectorXd&  Flocal2)
{
      double  velo[3];
...
int BernsteinElem3DINSTetra10Node::MassMatrices(vector<myPoint>& node_coords, double* elemData, VectorXd&  Mlocal1, VectorXd&  Mlocal2)
{
    int  ii;
...
//
double  BernsteinElem3DINSTetra10Node::CalculateError(vector<myPoint>& node_coords, double* elemData, double* timeData, VectorXd& veloPrev, VectorXd& accePrev, VectorXd& presPrev, double timeCur, int index)
{
...
int BernsteinElem3DINSTetra10Node::toComputeInfSupCondition(vector<myPoint>& node_coords, double* elemData, MatrixXd& Kuu, MatrixXd& Kup, MatrixXd& Kpp)
{
//
...
int  BernsteinElem3DINSTetra10Node::CalculateForces(int side, vector<myPoint>& node_coords, double* elemData, double* timeData, VectorXd& veloVec, VectorXd& presVec, VectorXd&  Flocal1)
{
