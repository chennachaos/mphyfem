...
int femMagnetomech::setSolver(int slv)
{
    if(debug)  PetscPrintf(PETSC_COMM_WORLD, "femMagnetomech::setSolver ... STARTED \n");
    ...
    if(debug)  PetscPrintf(PETSC_COMM_WORLD, "femMagnetomech::setSolver ... ENDED \n");
...
int femMagnetomech::prepareMatrixPattern()
{
    PetscPrintf(MPI_COMM_WORLD, "\n\n     femMagnetomech::prepareMatrixPattern()  .... STARTED ...\n\n");
...
int femMagnetomech::setSolverDataForSemiImplicit()
{
/*
...
int femMagnetomech::solveFullyImplicit()
{
    if(ntotdofs_global == 0)
      return 0;
    ...
    {
        cout << " femMagnetomech::solveFullyImplicit() ... Algorithm type is not available " << endl;
    }
...
int femMagnetomech::solveWithNewtonRaphson()
{
    cout << "\femMagnetomech::solveWithNewtonRaphson \n\n\n" << endl;
    ...
    while( (myTime.cur <= (timeFinal-EPSILON)) && (stepsCompleted <= stepsMax) )
    {
        ...
        for(int iter=1; iter <= iterationsMax; iter++)
        {
            ...
            PetscPrintf(MPI_COMM_WORLD, " femMagnetomech ...  %3d \t %11.4e \n", (iter), rhsNorm);

            // check for convergence and divergence of the iterations
            if( converged() )
            {
              PetscPrintf(MPI_COMM_WORLD, "\n femMagnetomech ...  Iterations CONVERGED \n\n\n");
            ...
            {
              PetscPrintf(MPI_COMM_WORLD, " femMagnetomech ...  Iterations are diverging. NR loop is terminated. \n\n\n");
              break;
            }
...
int femMagnetomech::solveWithArclength()
{
...
int femMagnetomech::calcStiffnessAndResidual()
{
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "     femMagnetomech::calcStiffnessAndResidual ...STARTED \n\n");}
    ...
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "     femMagnetomech::calcStiffnessAndResidual ... ENDED \n\n");}
...
int femMagnetomech::factoriseSolveAndUpdate()
{
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "     femMagnetomech::factoriseSolveAndUpdate ... STARTED \n\n");}
    ...
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "     femMagnetomech::factoriseSolveAndUpdate ... ENDED \n\n");}

/*
    if(debug) {cout << "     femMagnetomech::factoriseSolveAndUpdate ... STARTED \n\n";}
