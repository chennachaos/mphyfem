...
int femMagnetomech::prepareInputData()
{
    if(debug) PetscPrintf(PETSC_COMM_WORLD, "\n     femMagnetomech::prepareInputData()  .... STARTED ...\n");
...
int femMagnetomech::processForBernsteinElements()
{
/*
...
int  femMagnetomech::prepareDataForMixedElements()
{
  int  idd = ElementTypeDataList[0]->getElemTypeNameNum();
...
int  femMagnetomech::prepareDataForPressure()
{
/*
...
/*
void  femMagnetomech::prepareDataForPressure()
{
    ...
    {
      cerr << " Wrong element type in 'femMagnetomech::prepareDataForPressure()' " << endl;
      //exit(999);
    }
...
int  femMagnetomech::prepareDataForTemperature()
{
/*
    if(tempDegree ==  -1)
    {
...
int femMagnetomech::setInitialConditions()
{
    PetscPrintf(MPI_COMM_WORLD, "femMagnetomech::setInitialConditions() ... \n");
    if(debug) cout <<  " femMagnetomech::setInitialConditions() ... STARTED " <<  endl;
...
int femMagnetomech::setBoundaryConditions()
{
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "\n\n femMagnetomech::setBoundaryConditions() ... STARTED \n");}
    ...
    for(auto&  bc : boundaryConitions)
    {
        ...
        {
            throw runtime_error("Patch type not available in femMagnetomech::setBoundaryConditions");
        }
    }
    ...
    */
    ...
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "\n\n femMagnetomech::setBoundaryConditions() ... ENDED \n");}
...
int femMagnetomech::addExternalForces()
{
    if(debug) {PetscPrintf(MPI_COMM_WORLD, "\n\n femMagnetomech::addExternalForces() ... STARTED \n");}
    ...
    if(debug) {cout << endl; cout << endl; cout <<  " femMagnetomech::addExternalForces() ... ENDED " <<  endl;}
...
int femMagnetomech::updateIterStep()
{
...
int femMagnetomech::saveSolution()
{
    SolnData.saveSolution();
...
int femMagnetomech::copyElemInternalVariables()
{
  // copy internal variables intVar1 to intVar2
...
bool femMagnetomech::diverging(double factor)
{
...
int femMagnetomech::setSpecifiedDOFs_Displacement(vector<vector<bool> >&  NodeDofType)
{
  if(debug)  PetscPrintf(PETSC_COMM_WORLD, "femMagnetomech::setSpecifiedDOFs_Displacement() ... STARTED \n");
  ...
  dofs_specified_disp.clear();
    ...
    for(auto&  bc : boundaryConitions)
    {
        ...
        {
            throw runtime_error("Patch type not available in femMagnetomech::setSpecifiedDOFs");
        }
    }
    ...
    if(debug)  PetscPrintf(PETSC_COMM_WORLD, "femMagnetomech::setSpecifiedDOFs_Displacement() ... ENDED \n");
