...
int femCahnHilliard::setSolver(int slv, int *parm, bool cIO)
{
    PetscPrintf(MPI_COMM_WORLD, "\n\n     femCahnHilliard::setSolver()  .... STARTED ...\n\n");
    ...
    PetscPrintf(MPI_COMM_WORLD, "\n\n     femCahnHilliard::setSolver()  .... ENDED ...\n\n");
...
int femCahnHilliard::prepareMatrixPattern()
{
    PetscPrintf(MPI_COMM_WORLD, "\n\n     femCahnHilliard::prepareMatrixPattern()  .... STARTED ...\n\n");
              ...
              for(jj=0;jj<nsize;jj++)
              {
                if(forAssyVec[jj] != -1)
                {
                  //printf("ii.... %5d \t %5d \t %5d \t %5d \n",ii, jj, r, forAssyVec[jj]);
                    forAssyMat[r].insert(forAssyVec[jj]);
                }
              }
              }
            }
    ...
    PetscPrintf(MPI_COMM_WORLD, " femCahnHilliard::prepareMatrixPattern()  .... FINISHED. Took %f  milliseconds \n", (tend-tstart)*1000);
...
int  femCahnHilliard::solveFullyImplicit()
{
...
int femCahnHilliard::calcForcesOnBoundaries()
{
...
int femCahnHilliard::solveTimeStep()
{
    int  ii, jj;
    ...
    VecScatterCreateToAll(solverPetsc->solnVec, &ctx, &vecseq);
        ...
        for(int iter=1; iter<=iterationsMax; iter++)
        {
            ...
            if(rhsNorm < conv_tol)
            {
                PetscPrintf(MPI_COMM_WORLD, " Cahn-Hilliard : Solution converged below the specified tolerance. \n\n");
...
int femCahnHilliard::calcStiffnessAndResidual()
{
    if(mesh->getNDIM() == 2)
      return calcStiffnessAndResidualImplicit2D();
    else
      return calcStiffnessAndResidualImplicit3D();
...
int femCahnHilliard::calcStiffnessAndResidualImplicit2D()
{
    int  aa, ee, ii, jj, row, col, ind, size1, size2, npElem;
    int  gp, nGP, TI, TIp1, TIp2, TJ, TJp1, TJp2, degree;
    ElementShape  elShape;

    double  dvol, b1, b2, b3, b4, c1, c2, c4, xx, yy, Jac;
    double  phi, dphi[2], phiDot, eta, deta[2], MobFun, dMobFun, vel[2], force[2];
    double  fact, fact1, fact2, elemVol, charlen, param[2];
    double  funcf, dfuncf, eps, epsSq;
    double  WellHeight=0.25, constD=0.001;
    ...
    if(elem_proc_id[ee] == this_mpi_proc)
    {
        ...
        for(gp=0; gp<nGP; gp++)
        {
            ...
            {
              cout << " Jac = " << Jac << endl;
              cout << " Negative Jacobian in 'femCahnHilliard::calcStiffnessAndResidualLinearised2D' " << endl;
              exit(1);
            }
        ...
        for(gp=0; gp<nGP; gp++)
        {
            // compute variables and their derivatives

            xx = 0.0; yy = 0.0;
            phi = 0.0;  dphi[0] = 0.0;  dphi[1] = 0.0;  phiDot = 0.0;
            eta = 0.0;  deta[0] = 0.0;  deta[1] = 0.0;
            ...
            for(ii=0; ii<npElem; ii++)
            {
                ...
                phi     += b1*Nv[gp][ii];
                dphi[0] += b1*dNvdx[gp][ii];
                dphi[1] += b1*dNvdy[gp][ii];

                eta     += b2*Nv[gp][ii];
                deta[0] += b2*dNvdx[gp][ii];
                deta[1] += b2*dNvdy[gp][ii];

                phiDot  += solnDotCur[TI]  *Nv[gp][ii];
            }
            ...
            if(MULTIPHASEFLOW)
            {
              for(ii=0; ii<npElem; ii++)
              {
                ...
                vel[0] += veloConvection[TI];
                vel[1] += veloConvection[TIp1];
              }
            }

            //funcf   = chemicalPotential(1, WellHeight, phi);
            //dfuncf  = chemicalPotentialDerivative(1, WellHeight, phi);

            funcf   = 4.0*WellHeight*(phi*phi*phi-phi);
            dfuncf  = 4.0*WellHeight*(3.0*phi*phi-1.0);

            //MobFun  = mobilityFunction(1, constD, phi);
            //dMobFun = mobilityFunctionDerivative(1, constD, phi);
            ...
            for(ii=0;ii<npElem;ii++)
            {
                ...
                for(jj=0;jj<npElem;jj++)
                {
                    ...
                    // equation 1 - phi
                    //
                    // time acceleration term + advection term
                    Klocal(TI,   TJ)   += ( b4*acceFact*Nv[gp][jj] + b4*vel[0]*c1 + b4*vel[1]*c2);

                    // coupling term
                    Klocal(TI,   TJp1) += ( b1*MobFun*c1 + b2*MobFun*c2);
                    // coupling term linearisation
                    Klocal(TI,   TJ)   += ( b1*dMobFun*deta[0] + b2*dMobFun*deta[1])*c4;

                    // equation 2 - eta
                    //
                    // coupling term
                    Klocal(TIp1, TJ)   -= ( b4*dfuncf*c4 + b1*epsSq*c1 + b2*epsSq*c2 );

                    // diagonal term
                    Klocal(TIp1, TJp1) += b4*c4;
                }

                Flocal(TI)   -= (b4*phiDot + b4*(vel[0]*dphi[0]+vel[1]*dphi[1]) + b1*MobFun*deta[0] + b2*MobFun*deta[1] );
                Flocal(TIp1) -= (b4*(eta-funcf) - b1*epsSq*dphi[0] - b2*epsSq*dphi[1] );
            }
        } //gp
...
int femCahnHilliard::calcStiffnessAndResidualImplicit3D()
{
    int  aa, ee, ii, jj, row, col, ind, size1, size2, npElem;
    ...
    if(elem_proc_id[ee] == this_mpi_proc)
    {
        ...
        for(gp=0; gp<nGP; gp++)
        {
            ...
            {
              cout << " Jac = " << Jac << endl;
              cout << " Negative Jacobian in 'femCahnHilliard::calcStiffnessAndResidualLinearised3D' " << endl;
              exit(1);
            }
...
int femCahnHilliard::addExternalForces(double loadFact)
{
    int  nn, dof, ii, ind;
    double specVal=0.0;
...
int femCahnHilliard::computeElementErrors(int ind)
{
