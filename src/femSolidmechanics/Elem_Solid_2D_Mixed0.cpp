...
void Elem_Solid_2D_Mixed0::prepareElemData()
{
  ...
  {
     throw runtime_error("Elem_Solid_2D_Mixed0::prepareElemData() ... invalid element type");
  }
  ...
  FlocalPres.resize(nlbfP);

  presDOF.resize(nlbfP); presDOF.setZero();
  presDOFprev  = presDOF;
...
int Elem_Solid_2D_Mixed0::calcMassMatrix(MatrixXd& Mlocal, bool MassLumping)
{
  return 0;
}
...
int Elem_Solid_2D_Mixed0::calcStiffnessAndResidual(MatrixXd& Klocal, VectorXd& Flocal)
{
    ElementBase::calcStiffnessAndResidualMixed2D(Klocal, Kup, Kpu, Kpp, Flocal, FlocalPres);

    // add contribution from the condensed matrix

    Klocal -=  ( Kup*(Kpu/Kpp(0,0)) );
    Flocal -=  ( Kup*(FlocalPres/Kpp(0,0)) );
...
int Elem_Solid_2D_Mixed0::solveForPressure()
{
    double  volstrain = 0.0, volstrainPrev=0.0;

    // explicit scheme
    /*
    double eps  = MatlData->getKinv();
    double BULK = 1.0/eps;
    if(SolnData->tis >= 400)
    {
      if(finite)
    ...
    */
      ...
      for(ii=0; ii<npElem; ii++)
      {
        ...
        for(kk=0; kk<2; kk++)
        {
          dispIncr(ind1+kk)  =  SolnData->dispIncr[ind2+kk];
        }
      }

      VectorXd  vtmp = Kpu*dispIncr;

      presDOF[0] += (FlocalPres(0) - vtmp(0))/Kpp(0,0);
    //}

    //cout <<  "pres = " <<  presDOF[0] <<  endl;
...
void Elem_Solid_2D_Mixed0::elementContourplot(int vartype, int varindex, int index)
{
   double outval[50];
   ...
   switch(vartype)
   {
       ...
       default:
              ...
              cout  << " Invalid Variable Type to project in 'Elem_Solid_2D_Mixed0::projectToNodes'" << endl;
              break;
    }
...
void Elem_Solid_2D_Mixed0::projectToNodes(bool extrapolateFlag, int vartype, int varindex, int index)
{
   double outval[50];
...
void Elem_Solid_2D_Mixed0::projectStress(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
    int  err = 0, ii, jj, gp;
...
void Elem_Solid_2D_Mixed0::projectStrain(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
  return;
}
...
void Elem_Solid_2D_Mixed0::projectInternalVariable(bool extrapolateFlag, int vartype, int varindex, int index, double* outval)
{
